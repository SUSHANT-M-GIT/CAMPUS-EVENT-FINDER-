...
export default function QrScannerModal({ eventId, eventTitle, certificatesEnabled: certEnabledProp, onClose }: Props) {
  ...
  const loadList = async () => {
    setListLoading(true);
    try { const data = await getAttendanceList(eventId); setAttendanceList(data); }
    catch { /* silent */ }
  ...
  const processQrData = async (rawData: string) => {
    try {
      // Support both JSON payload (new) and raw registrationId string (old QRs)
      ...
      try {
        ...
        // Not JSON — treat the raw value as the ID/code directly
        registrationId = rawData.trim();
      }
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!code) { setScanResult({ msg: "Could not decode QR from image. Try a clearer photo.", type: "error" }); return; }
      await processQrData(code.data);
    } catch (err: any) {
      setScanResult({ msg: "Error: " + err.message, type: "error" });
    } finally {
      setScanning(false);
  ...
  return (
    <div
      style={{ position: "fixed", inset: 0, background: "rgba(2,48,71,0.65)", display: "grid", placeItems: "center", zIndex: 200, backdropFilter: "blur(4px)", padding: 16 }}
      onClick={onClose}
    >
      <div
        style={{ width: "min(640px,100%)", background: "var(--surface-2)", borderRadius: 18, overflow: "hidden", boxShadow: "0 24px 60px rgba(0,0,0,0.3)", maxHeight: "90vh", display: "flex", flexDirection: "column" }}
      ...
      >
        ...
        <div style={{ background: "linear-gradient(135deg,#023047,#1e3a5f)", padding: "18px 24px", color: "#fff", flexShrink: 0 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <button onClick={onClose} style={{ background: "rgba(255,255,255,0.15)", border: 0, borderRadius: 8, padding: "6px 12px", color: "#fff", cursor: "pointer", fontSize: "0.85rem" }}>x Close</button>
          </div>
        </div>
        ...
        <div style={{ padding: "20px 24px", overflowY: "auto", flex: 1 }}>
          ...
          <div style={{ marginBottom: 16 }}>
            <p style={{ margin: "0 0 8px", fontSize: "0.82rem", fontWeight: 600, color: "var(--text-2)" }}>Live Camera Scan</p>

            {/* Video element always mounted so ref is always available */}
            <div style={{ position: "relative", borderRadius: 10, overflow: "hidden", background: "#000", marginBottom: 10, display: cameraActive ? "block" : "none" }}>
              <video ref={videoRef} playsInline muted style={{ width: "100%", display: "block", maxHeight: 260, objectFit: "cover" }} />
              <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}>
                <div style={{ width: 180, height: 180, border: "3px solid rgba(99,102,241,0.9)", borderRadius: 12, boxShadow: "0 0 0 9999px rgba(0,0,0,0.35)" }} />
              </div>
              <div style={{ position: "absolute", bottom: 8, left: 0, right: 0, textAlign: "center" }}>
                <span style={{ background: "rgba(0,0,0,0.6)", color: "#fff", fontSize: "0.75rem", padding: "3px 10px", borderRadius: 99 }}>
            ...
            {cameraError && (
              <div style={{ background: "#fee2e2", color: "#991b1b", borderRadius: 8, padding: "10px 14px", fontSize: "0.84rem", marginBottom: 8 }}>
                {cameraError}
              </div>
            )}
            ...
            {!cameraActive ? (
              <button
                type="button" onClick={startCamera} disabled={scanning}
              ...
              <button
                ...
                style={{ width: "100%", background: "#dc2626", color: "#fff", border: 0, borderRadius: 10, padding: "10px", fontWeight: 700, cursor: "pointer", fontSize: "0.88rem" }}
              >
                Stop Camera
              </button>
            )}
          ...
          <div style={{ borderTop: "1px solid var(--border)", paddingTop: 14, marginBottom: 16 }}>
            <p style={{ margin: "0 0 8px", fontSize: "0.82rem", fontWeight: 600, color: "var(--text-2)" }}>Upload QR Image</p>
            <input ref={fileRef} type="file" accept="image/*" onChange={handleFileUpload} style={{ display: "none" }} />
            <button
              type="button" onClick={() => fileRef.current?.click()} disabled={scanning || cameraActive}
              style={{ width: "100%", background: (scanning || cameraActive) ? "#94a3b8" : "rgba(99,102,241,0.15)", color: (scanning || cameraActive) ? "#fff" : "#818cf8", border: "1px solid rgba(99,102,241,0.3)", borderRadius: 10, padding: "10px", fontWeight: 700, cursor: (scanning || cameraActive) ? "not-allowed" : "pointer", fontSize: "0.88rem" }}
            >
          ...
          <div style={{ borderTop: "1px solid var(--border)", paddingTop: 12, marginBottom: 20 }}>
            <p style={{ margin: "0 0 8px", fontSize: "0.82rem", fontWeight: 600, color: "var(--text-2)" }}>Manual Registration Code</p>
            <div style={{ display: "flex", gap: 8 }}>
              <input
                ...
                style={{ flex: 1, border: "1px solid var(--border)", borderRadius: 9, padding: "9px 12px", fontSize: "0.88rem", outline: "none", fontFamily: "monospace", letterSpacing: "0.05em" }}
              />
              <button
                type="button" onClick={handleManualScan} disabled={!manualId.trim() || scanning}
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
            <button
              type="button" onClick={loadList} disabled={listLoading}
              style={{ flex: 1, background: "rgba(255,255,255,0.06)", color: "var(--text-2)", border: 0, borderRadius: 9, padding: "9px", fontWeight: 600, cursor: "pointer", fontSize: "0.85rem" }}
            >
              {listLoading ? "Loading..." : "Refresh Attendance List"}
            </button>
            {!certificatesEnabled && (
              <button
                type="button" onClick={handleEnableCerts} disabled={certLoading}
            ...
            {certificatesEnabled && (
              <div style={{ flex: 1, background: "rgba(34,197,94,0.15)", color: "var(--success)", borderRadius: 9, padding: "9px", fontWeight: 600, fontSize: "0.85rem", textAlign: "center" }}>
                Certificates Active
              </div>
            )}
          </div>
                ...
                <thead>
                  <tr style={{ background: "var(--surface)", position: "sticky", top: 0 }}>
                    {["Student", "College", "Status"].map(h => (
                      <th key={h} style={{ padding: "9px 12px", textAlign: "left", color: "var(--text-2)", fontWeight: 600, borderBottom: "1px solid var(--border)" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {attendanceList.map((r, i) => (
                    <tr key={r._id} style={{ borderBottom: "1px solid var(--border)", background: i % 2 === 0 ? "var(--surface)" : "var(--surface-2)" }}>
                      <td style={{ padding: "8px 12px", fontWeight: 600, color: "var(--text)" }}>{r.studentName}</td>
                      <td style={{ padding: "8px 12px", color: "var(--text-muted)" }}>{r.collegeName || ""}</td>
