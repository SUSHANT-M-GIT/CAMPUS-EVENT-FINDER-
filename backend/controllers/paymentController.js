 ...
 * Flow:
 *   1. createRazorpayOrder  → backend creates order, returns order_id
 *   2. Frontend opens Razorpay checkout popup
 *   3. Student pays → Razorpay calls verifyRazorpayPayment with signature
 *   4. Backend verifies signature → marks approved → sends QR email
 */
...
// Razorpay instance — reads keys from .env at call time
...
// ── Shared helper: approve a registration after payment confirmed ─────────────
...
// ── VERIFY RAZORPAY PAYMENT (student, called after checkout success) ──────────
...
// ── SUBMIT MANUAL PAYMENT (student — fallback if Razorpay fails) ──────────────
