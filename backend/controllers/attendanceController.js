...
exports.scanAttendance = async (req, res) => {
  try {
    ...
    if (reg.attendanceStatus === 'present')
      return res
        .status(400)
        .json({ msg: '⚠ Attendance already marked for this student', alreadyScanned: true });
...
exports.enableCertificates = async (req, res) => {
  try {
    ...
    res.json({
      msg: `Certificates enabled. ${presentRegs.length} student(s) assigned certificate IDs.`,
    });
  } catch (e) {
    res.status(500).json({ msg: e.message });
  }
...
exports.downloadCertificate = async (req, res) => {
  try {
    ...
    if (reg.attendanceStatus !== 'present')
      return res
        .status(400)
        .json({ msg: 'Certificate is only available for students who attended the event' });

    const event = reg.eventId;
    if (!event?.certificatesEnabled)
      return res
        .status(400)
        .json({ msg: 'Certificate generation has not been enabled for this event yet' });
    ...
    doc
      .moveTo(W / 2 - nameWidth / 2, 256)
      .lineTo(W / 2 + nameWidth / 2, 256)
      .lineWidth(2)
      .stroke('#4f46e5');
