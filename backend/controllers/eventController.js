...
function extractBannerFields(req) {
  ...
  if (gdriveLink) {
    ...
    if (!direct)
      return {
        _gdriveError:
          'Invalid Google Drive link format. Use: https://drive.google.com/file/d/FILE_ID/view',
      };
  ...
  try {
    ...
    if (newImageFile && e.bannerSource === 'local' && e.bannerImage) {
      ...
      fs.unlink(oldPath, (err) => {
        if (err) console.warn('[Upload] Could not delete old banner:', err.message);
      });
    }
  ...
  try {
    ...
    if (e.bannerSource === 'local' && e.bannerImage) {
      ...
      fs.unlink(filePath, (err) => {
        if (err) console.warn('[Upload] Could not delete banner:', err.message);
      });
    }
